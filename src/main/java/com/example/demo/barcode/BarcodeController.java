...
public class BarcodeController {
    ...
    public ResponseEntity<BarcodeResponse> handleBarcodeScanned(@RequestBody BarcodeRequest request) {
        log.info("Barcode scanned: {} with type: {}", request.getBarcodeValue(), request.getScanType());
    ...
    private ResponseEntity<BarcodeResponse> handlePackageStart(String packageId) {
        ...
        PackageStateService.PackageState packageState = packageStateService.startPackage(packageId, presignedUrl);
        
        BarcodeResponse response = BarcodeResponse.builder()
                .success(true)
                .message("Package recording started. Begin scanning items to verify.")
    ...
    private ResponseEntity<BarcodeResponse> handleItemVerification(String itemBarcode) {
        log.info("Processing item verification for item: {}", itemBarcode);
        
        // Get the active package
        PackageStateService.PackageState activePackage = packageStateService.getAnyActivePackage();
        ...
        log.info("Verifying item {} against package {}", itemBarcode, activePackage.getPackageId());
        
        BarcodeResponse response = BarcodeResponse.builder()
                .success(true)
                .message(String.format("Item %s verified for package %s", itemBarcode, activePackage.getPackageId()))
                .barcodeValue(itemBarcode)
    ...
    public ResponseEntity<BarcodeResponse> endPackage(@RequestParam String packageId) {
